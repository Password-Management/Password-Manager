...
type Login interface {
	...
	ReLogin(userId uuid.UUID) error
}
...
func (ls *LoginImpl) ReLogin(userId uuid.UUID) error {
	db, err := db.NewDbRequest()
	if err != nil {
		log.Println("error in creating a DB request")
		return nil
	...
	updateDetails := transaction.Model(models.DBLogin{}).Where("user_id = ?", userId).Update("is_login", true)
...
func (ls *LoginImpl) Delete(userId uuid.UUID) error {
	...
	defer transaction.Rollback()
	deleteUser := transaction.Unscoped().Where("user_id=?", userId).Delete(&models.DBLogin{
		UserId: userId,
	})
